...
def Kitti_eigen_test_improved(split, **kwargs):
    ...
    with open("./Datasets/split/eigen_test_improved.txt") as eigen_test_improved_file:
        ...
        for row in eigen_test_improved_reader:
            ...
            if (
                os.path.isfile(inputleft)
                and os.path.isfile(inputright)
                and os.path.isfile(groundtruthleft)
                and os.path.isfile(velodyneleft)
            ):
        ...
        if len(test_list) != 652:
            raise Exception(
                f'Could only load {len(test_list)} images from "KITTI eigen test improved split" of size 652.'
            )
