# Train_Stage1_Kslow.py Train model on KITTI, using both Left and right images
...
parser.add_argument('--milestones', default=[30, 40], metavar='N', nargs='*',
...
parser.add_argument('--sparse', default=True, action='store_true',
                    help='Depth GT is sparse, automatically selected when choosing a KITTIdataset')
...
#                     default='Kitti_stage1\\10-14-14_48\\FAL_netB,e50es,b8,lr0.0001\\checkpoint.pth.tar',
...
def train(train_loader, m_model, g_optimizer, epoch):
    ...
    for i, input_data0 in enumerate(train_loader):
        ...
        pan, disp = m_model(torch.cat((left_view, F.grid_sample(right_view, flip_grid, align_corners=True)), 0),
        ...
        rec_loss = (rec_loss_fnc(mask, rpan, right_view, vgg_right, args.a_p) +
                    rec_loss_fnc(mask, lpan, left_view, vgg_left, args.a_p)) / 2
