# FAL_netB.py This script contains the network architecture of FAL_netB
    ...
    def forward(self, input_left, min_disp, max_disp, ret_disp=True, ret_subocc=False, ret_pan=False):
        ...
        for n in range(0, self.no_levels * self.no_fac):
            ...
            if n == 0:
                ...
                Dprob = torch.cat((Dprob, F.grid_sample(dlog0[:, n, :, :].unsqueeze(1), out_grid, align_corners=True)), 1)
        ...
        for n in range(0, self.no_levels * self.no_fac):
            ...
            # You could get disp in the right (opposite)
            # w = max_disp * torch.exp(torch.log(max_disp / min_disp) * (c - 1))
            # dispr = dispr + w.unsqueeze(1) * Dprob[:, n, :, :].unsqueeze(1)

            # Get synth right view
            if ret_pan:
                p_im0 = p_im0 + \
                        F.grid_sample(input_left, out_grid, align_corners=True) * Dprob[:, n, :, :].unsqueeze(1)
